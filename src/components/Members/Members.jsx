...
export default function Members() {
    ...
    const sourcemembers = Members.length > 0 ? Members : studentsData;
    // بعد فیلتر و سرچ رو روی sourceStudents بزن
    const filteredAndSortedmembers = useMemo(() => {
    return sourcemembers.filter(student =>
        (student.full_name || student.name || '')
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
    );
    }, [sourcemembers, searchTerm]);
    // شمارش درست
    const absentmembers = filteredAndSortedmembers.filter(s => s.absent === true);
    const presentmembers = filteredAndSortedmembers.filter(s => s.absent === false);
  ...
  return (
    <div className={style.container}>
        <div className={style.header}>
            ...
            <h1>لیست کل کادر آموزشگاه ها</h1>
                <div className={style.p_dad}>
                    <p>{!loading && `${absentmembers.length} معاون غایب هستند`}</p>
                    <p>{!loading && `${presentmembers.length} معاون حاضر هستند`}</p>
                </div>
            </div>
            <div className={style.left}>
                <Dropdown options={options} defualt={"به ترتیب کد ملی"} onSelect={handleSelect} />
        ...
        <div className={style.table}>
            {filteredAndSortedmembers.map((s, index) => (
