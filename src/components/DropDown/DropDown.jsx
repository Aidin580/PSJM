...
const Dropdown = ({ options, onSelect,defualt }) => {
...
useEffect(() => {
  ...
  return (
    <div ref={dropdownRef} className={style.main}>
      <img className={style.sort} src={Sort} alt="" />
    <div onClick={toggleDropdown} className={style.container}>
      <img   className={isOpen ? style.rotated : ""} src={drop} alt="" />
      <button className={style.button}>
        {selected ? selected.label : defualt}
      </button>
    </div>  
        <ul
  className={`${style.options} ${isOpen ? style.show : style.hide}`}
>
  {options.map((option) => (
    <li
