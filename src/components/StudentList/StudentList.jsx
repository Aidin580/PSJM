...
export default function StudentList() {
  ...
  const [sortBy, setSortBy] = useState("national_id");
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        ...
        // **FIX:** خواندن آرایه دانش‌آموزان از response.data.data
  ...
  const filteredAndSortedStudents = useMemo(() => {
    ...
    return students
      .filter(student => {
        // **FIX:** استفاده از فیلد full_name برای جستجوی بهتر
  ...
  const handleDelete = (studentId) => {
    console.log(`درخواست حذف برای دانش‌آموز با آیدی ${studentId}`);
  };

  const handleEdit = (studentId) => {
    console.log(`درخواست ویرایش برای دانش‌آموز با آیدی ${studentId}`);
  };

  const classOptions = [
    { value: "all", label: "تمامی کلاس ها" },
    { value: "g10_network", label: "دهم شبکه" },
    { value: "g10_accounting", label: "دهم حسابداری" },
    { value: "g11_network", label: "یازدهم شبکه" },
  ];
  
  const sortOptions = [
    { value: "national_id", label: "به ترتیب کد ملی" },
    { value: "lastname", label: "به ترتیب نام خانوادگی" },
    { value: "firstname", label: "به ترتیب نام" },
  ];
  ...
  const renderStudentRows = () => {
    if (loading) {
      return Array.from({ length: 5 }).map((_, index) => (
        <div key={index} className={style.row}>
          <div className={style.item}><Skeleton duration={2}  highlightColor="#69b0b2" width={"100%"}  /></div>
          <div className={`${style.item} ${style.display}`}><Skeleton duration={2}  highlightColor="#69b0b2" width={120} /></div>
          <div className={`${style.item} ${style.display}`}><Skeleton duration={2}  highlightColor="#69b0b2" width={120} /></div>
          <div className={style.item}><Skeleton duration={2}  highlightColor="#69b0b2" width={100} /></div>
          <div className={style.item}><Skeleton duration={2}  highlightColor="#69b0b2" width={50} /></div>
          <div className={style.delete}>حذف</div>
          <div className={`${style.edit} ${style.display}`}>ویرایش</div>
        </div>
      ));
    }
    ...
    if (filteredAndSortedStudents.length === 0) {
      return <div className={style.emptyState}><p>دانش‌آموزی با این مشخصات یافت نشد.</p></div>;
    }

    return filteredAndSortedStudents.map(student => {
      const isAbsent = absentList.some(s => s.id === student.id);
      return (
        <div key={student.id} className={style.row}>
          {/* **FIX:** استفاده از نام فیلدهای صحیح از API */}
          <div className={style.item}>
            <p>{student.first_name || 'بی‌نام'}</p>
          </div>
          <div className={`${style.item} ${style.display}`}>
            <p>{student.last_name || 'بی‌نام خانوادگی'}</p>
          </div>
          <div className={`${style.item} ${style.display}`}>
            <p>نام پدر: {student.father_name || '---'}</p>
          </div>
          <div className={style.item}>
            <p><span className={style.display}></span>{student.national_code || '---'}</p>
          </div>
          <div className={style.item} >
            <div className={isAbsent ? style.checkboxAbsent : style.checkbox}></div>
            <p>{isAbsent ? <span className={style.absentText}>غایب است</span> : "حاضر است"}</p>
          </div>
          <div className={style.delete} onClick={() => handleDelete(student.id)}>
            <img src={deleteIcon} alt="حذف" />
            <p>حذف</p>
          </div>
          <div className={`${style.edit} ${style.display}`} onClick={() => handleEdit(student.id)}>
    ...
    <div className={style.container}>
      <div className={style.header}>
        <div className={style.right}>
          ...
          <p>{!loading && `${filteredAndSortedStudents.length} دانش‌آموز یافت شد`}</p>
        </div>
        <div className={style.left}>
          <div className={`${style.filter} ${style.display}`}>
            <Dropdown options={classOptions} defualt={"تمامی کلاس ها"} onSelect={(option) => setSelectedClass(option.value)} />
          </div>
          <div className={style.filter}>
            <Dropdown options={sortOptions} defualt={"به ترتیب کد ملی"} onSelect={(option) => setSortBy(option.value)} />
          </div>
