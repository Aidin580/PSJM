...
export default function StudentList() {
  ...
  const studentsData = [
    ...
    { name: "صدرا", lastName: "شعبان نژاد", nationalCode: "----------", school: "دبیرستان غیر انتفاعی دین و دانش", absent: true },
  ...
  const sourceStudents = students.length > 0 ? students : studentsData;
  ...
  // شمارش درست
  const absentStudents = filteredAndSortedStudents.filter(s => s.absent === true);
  const presentStudents = filteredAndSortedStudents.filter(s => s.absent === false);
  ...
  const classOptions = [
    ...
    { value: "first_grade", label: "اول" },
    { value: "second_grade", label: "دوم" },
    { value: "third_grade", label: "سوم" },
    { value: "fourth_grade", label: "چهارم" },
    { value: "fifth_grade", label: "پنج" },
    { value: "sixth_grade", label: "ششم" },
    { value: "seventh_grade", label: "هفتم" },
    { value: "eighth_grade", label: "هشتم" },
    { value: "ninth_grade", label: "نهم" },
    { value: "tenth_grade", label: "دهم" },
    { value: "eleventh_grade", label: "یازدهم" },
    { value: "twelfth_grade", label: "دوازدهم" },
  ];
    ...
    <div className={style.container}>
      <div className={style.header}>
        <div className={style.right}>
          ...
          <h1>لیست کل دانش آموزان</h1>
          <div className={style.p_dad}>
            <p>{!loading && `${absentStudents.length} دانش آموز غایب هستند`}</p>
            <p>{!loading && `${presentStudents.length} دانش آموز حاضر هستند`}</p>
          </div>
        </div>
        <div className={style.left}>
          <div className={style.alighner}>
            <div className={`${style.filter} ${style.display}`}>
              <Dropdown options={classOptions} defualt={"تمامی پایه ها"} onSelect={(option) => setSelectedClass(option.value)} />
            </div>
